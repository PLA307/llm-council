...
## 8. 工作流规范 (Workflow Standards)

为了保证生产环境的稳定性，我们遵循 **“本地先行，按需发布”** 的原则：

1.  **默认不推送**: AI 在修改代码后，默认**只在本地保存**，不自动执行 `git push`。
2.  **本地验证**: 
    *   修改后，AI 会协助启动本地服务 (`npm run dev` 或 `python -m backend.main`)。
    *   用户在本地浏览器 (`localhost`) 确认功能正常。
3.  **手动发布**: 
    *   只有当用户明确发出指令（如“提交代码”、“发布”、“推送到仓库”）时，AI 才会执行 `git push`。
    *   推送操作会自动触发 Vercel (前端) 和 Render (后端) 的生产环境部署。
